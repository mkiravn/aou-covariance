...
def estimates(S, N, r, mid):
    ...
    def fit(name, label, classes, lo, hi, design, readout):
        a, m, n = bins(classes, lo, hi)
    ...
    fit("h2_Unrel", "noPO", NOPO, -np.inf, 0.02, origin, lambda b: b[0])

    for label, classes in (("noPO", NOPO), ("pooled", POOLED), ("PO", ["PO"])):
        # classic sib regression -- slope conflates h2 and shared environment
        fit("h2_FS", label, classes, 0.4, 0.6, origin, lambda b: b[0])
        # shared environment among first-degree pairs
        fit("b2_FS", label, classes, 0.4, 0.6,
            lambda a: np.c_[a, np.ones_like(a)], lambda b: 2 * b[1])

    for label, classes in (("noPO", NOPO), ("pooled", POOLED)):
        # quadratic absorbs non-linearity (Wainschtein et al. 2025)
        fit("h2_PedW25", label, classes, 0.05, 0.7,
            lambda a: np.c_[a, a ** 2], lambda b: b[0])
        # intercept offsets at pedigree-class transitions
        fit("h2_Pedf", label, classes, 0.05, 0.7,
            lambda a: np.c_[a, _ind(a, .1, .2), _ind(a, .2, .4), _ind(a, .4, .6)],
            lambda b: b[0])
        # separate FS (0.4-0.6) and HS (0.2-0.4) intercepts, common slope
        fit("b2_step", label, classes, 0.2, 0.6, step, lambda b: 4 * (b[1] - b[2]))
        fit("b2_step_ratio", label, classes, 0.2, 0.6, step,
            lambda b: b[1] / b[2] if b[2] != 0 else np.nan)
