...
options(bigrquery.quiet = TRUE)   # suppress per-page download progress messages -- large
                                   # pulls (e.g. zip3_ses_map) can otherwise print fast enough
                                   # to trip Jupyter's IOPub rate limit ("IOPub data rate exceeded")
...
pull_phenotype <- function(row, keep_ids) {
  ...
  if (file.exists(cache_path)) {
    cached <- read_tsv(cache_path, col_types = cols(person_id = "c", phenotype = "d", age = "d", sex_at_birth = "c"))
    if (nrow(cached) > 0) return(cached)
    warning(sprintf("pull_phenotype(): ignoring stale empty cache for '%s', re-pulling", row$phenotype_name))
  }
  ...
  if (nrow(pheno_df) == 0) {
    warning(sprintf(
      "pull_phenotype(): '%s' returned 0 rows -- not caching. Check concept_id, REFERENCE_DATE, and that keep_ids actually overlaps person_id.",
      row$phenotype_name
    ))
  } else {
