...
practice. Reads `01_ancestry_pca_filter.ipynb`'s keep-list and `03_final_pca.ipynb`'s
...
for PCs; `03_final_pca.ipynb`'s file has 10, only the top 5 are used, since
...
`notebooks/remote/02_residualize_phenotypes.ipynb`: takes `03_final_pca.ipynb`'s
